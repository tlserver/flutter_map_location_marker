...
displaying the device's current location on a map. It provides a simple and flexible way to add a
...
- **Simple**: The only thing you need to do is to add a `CurrentLocationLayer()` to your map
...
- **Flexible**: The default implementation receives the device's position from
  ...
  the [flutter_rotation_sensor](https://pub.dev/packages/flutter_rotation_sensor) package, but with
  type conversion, streams from other sources are also supported.

- **Auto-following**: The map follows the new location when the location is updated. This feature is
  disabled by default.

- **Auto-rotating**: The map can be rotated automatically in navigation mode. This feature is
  disabled by default.

- **Customization**: The location marker can be fully customized, including the colors of the
  accuracy circle and heading sector.
...
4. [Selectable Distance Filter](https://github.com/tlserver/flutter_map_location_marker/blob/main/example/lib/page/selectable_distance_filter_example.dart):
   Change Geolocator settings at runtime.
...
*Q*: How can I get
the [positionStream](https://pub.dev/documentation/flutter_map_location_marker/latest/flutter_map_location_marker/CurrentLocationLayer/positionStream.html)
, [headingStream](https://pub.dev/documentation/flutter_map_location_marker/latest/flutter_map_location_marker/CurrentLocationLayer/headingStream.html)
or their origin streams from a CurrentLocationLayer widget?

*A*: You should not get these streams from a CurrentLocationLayer widget. Instead, create your own
streams with the same types and also pass them to all widgets that need them. CurrentLocationLayer
doesn't own the streams; it just listens to them. You can use any implementation of position and
heading streams, as long as the stream types are Stream&lt;LocationMarkerPosition?&gt; and
Stream&lt;LocationMarkerHeading?&gt;, respectively. You may
